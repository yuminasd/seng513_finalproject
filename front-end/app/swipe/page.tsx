...
function Swipe() {
    ...
    const handleLike = async () => {
        try {
            ...
            const movieId = movie.id; // Assuming you want to like the first movie
            try {
        ...
        <section className="flex flex-col h-screen overflow-hidden">
            ...
            <div className="flex h-[100%]">
                ...
                <motion.div ref={scope} className="overflow-hidden relative flex flex-col gap-1 bg-black rounded-md  w-full h-full aspect-video border border-neutral-900 p-4">
                    ...
                    <img className="object-contain h-[20rem] border-white border-2" src={movie.bgImg} alt={`${movie.name} background`} />
                    <p>Rating: {movie.rating}</p>
                    <p className=" w-full"> {movie.description}</p>
                    <div className='flex flex-row  gap-4   place-content-center w-full h-full pt-[15%]'>
