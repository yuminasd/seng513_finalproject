...
const MovieCard: React.FC<{ movie: Movie }> = ({ movie }) => {
    ...
    return (
        <div className="overflow-hidden relative flex flex-col gap-1 bg-black rounded-md  w-full h-48 aspect-video border border-neutral-900 p-4" onClick={toggleModal}>

            <img className="object-contain hover:opacity-50 rounded-md" src={movie.bgImg} alt={`${movie.name} background`} />
            <p className="text-xs  text-center text-neutral-500">{movie.name}</p>
            {/* Modal */}
            {openModal ? (
